// api/approve-review.js
// Called when you click the approval link in your email.
// Verifies the token, commits the review to reviews.js via GitHub API,
// then Vercel auto-deploys and the review goes live (~30 seconds).
...
module.exports = async function handler(req, res) {
  ...
  let review;
  try {
    review = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'));
  } catch {
  ...
  if (insertAt === -1) {
    return res.status(500).send('Could not find insertion point in reviews.js.');
  }
  ...
  if (!commitRes.ok) {
    ...
    return res.status(500).send('Failed to publish review. Check GitHub token permissions.');
  }

  // ── Success page (auto-redirects after 5s) ────────────────
...
<head>
  ...
  <title>Review Published — Taskboost.ai</title>
  <style>
...
<body>
  <div class="card">
    ...
    <p>
      <strong>${review.name}</strong>'s review is now live on Taskboost.ai.<br>
      Vercel will deploy the update in about 30 seconds.
    </p>
    <a href="${siteUrl}/#reviews">View Reviews →</a>
    <p class="note">Redirecting automatically in 5 seconds…</p>
  </div>
</body>
</html>`);
};
